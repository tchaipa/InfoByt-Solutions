import "./Projects.css";
import { useEffect, useState } from "react";

function Projects() {
  const [openIds, setOpenIds] = useState({});

  useEffect(() => {
    const btns = Array.from(document.querySelectorAll(".filter-btn"));
    if (!btns.length) return;

    const onFilter = (e) => {
      const btn = e.currentTarget;
      btns.forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      const filter = btn.dataset.filter;
      document.querySelectorAll(".project-card").forEach((card) => {
        if (filter === "all" || card.dataset.cat === filter) {
          card.style.display = "";
        } else {
          card.style.display = "none";
        }
      });
    };

    btns.forEach((b) => b.addEventListener("click", onFilter));
    return () => btns.forEach((b) => b.removeEventListener("click", onFilter));
  }, []);

  function toggleDetail(id) {
    setOpenIds((prev) => ({ ...prev, [id]: !prev[id] }));
  }

  return (
    <section id="projects">
      <div className="projects-section">
        <div className="reveal">
          <span className="section-label">Our Work</span>
          <h2 className="section-heading">Selected Projects</h2>
          <p className="section-sub">
            Real solutions built for real Zimbabwean businesses. Each project
            reflects our commitment to quality, performance, and measurable
            outcomes.
          </p>
        </div>
        <div className="projects-filter reveal">
          <button className="filter-btn active" data-filter="all">
            All
          </button>
          <button className="filter-btn" data-filter="web">
            Web Apps
          </button>
          <button className="filter-btn" data-filter="mobile">
            Mobile
          </button>
          <button className="filter-btn" data-filter="system">
            Systems
          </button>
          <button className="filter-btn" data-filter="erp">
            ERP
          </button>
        </div>
        <div className="projects-grid" id="projectsGrid">
          <div
            className="project-card reveal"
            data-cat="erp"
            data-id="retailpro"
          >
            <div className="project-thumb">🏪</div>
            <div className="project-body">
              <span className="project-tag tag-erp">ERP / POS</span>
              <h3>RetailPro — Multi-Branch Retail ERP</h3>
              <p>
                A complete point-of-sale and inventory management ERP for a
                Harare-based retail chain spanning 5 branches, with real-time
                stock sync, employee management, and detailed sales analytics.
              </p>
              <div className="project-stack">
                <span className="stack-pill">Django</span>
                <span className="stack-pill">React</span>
                <span className="stack-pill">PostgreSQL</span>
                <span className="stack-pill">Redis</span>
                <span className="stack-pill">Docker</span>
              </div>
              <button
                className="expand-btn"
                onClick={() => toggleDetail("retailpro")}
              >
                {openIds["retailpro"]
                  ? "Hide details ↑"
                  : "View full details ↓"}
              </button>
              <div
                className={
                  "project-detail " + (openIds["retailpro"] ? "open" : "")
                }
              >
                <div className="project-detail-inner">
                  <div className="detail-row">
                    <div className="detail-label">Client</div>
                    <div className="detail-val">
                      Confidential — Retail (FMCG)
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Duration</div>
                    <div className="detail-val">8 months (2023)</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Scope</div>
                    <div className="detail-val">
                      POS terminals, central inventory, HR module, financial
                      reporting, WhatsApp receipt delivery
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Impact</div>
                    <div className="detail-val">
                      40% reduction in stock discrepancies; 3× faster checkout
                      process
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Team size</div>
                    <div className="detail-val">
                      4 developers, 1 designer, 1 PM
                    </div>
                  </div>
                </div>
              </div>
              <div className="project-meta">
                <span>📅 2023</span>
                <span>👥 5 branches</span>
                <span>⚡ 40% efficiency gain</span>
              </div>
            </div>
          </div>

          <div className="project-card reveal" data-cat="web" data-id="finedge">
            <div className="project-thumb">🏦</div>
            <div className="project-body">
              <span className="project-tag tag-web">Web App</span>
              <h3>FinEdge — Microfinance Client Portal</h3>
              <p>
                A secure, full-featured web platform for a microfinance
                institution enabling clients to apply for loans, track
                repayments, and receive statements — reducing branch visits by
                over 60%.
              </p>
              <div className="project-stack">
                <span className="stack-pill">Next.js</span>
                <span className="stack-pill">Node.js</span>
                <span className="stack-pill">MySQL</span>
                <span className="stack-pill">AWS</span>
                <span className="stack-pill">Stripe</span>
              </div>
              <button
                className="expand-btn"
                onClick={() => toggleDetail("finedge")}
              >
                {openIds["finedge"] ? "Hide details ↑" : "View full details ↓"}
              </button>
              <div
                className={
                  "project-detail " + (openIds["finedge"] ? "open" : "")
                }
              >
                <div className="project-detail-inner">
                  <div className="detail-row">
                    <div className="detail-label">Client</div>
                    <div className="detail-val">
                      Microfinance institution, Harare
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Duration</div>
                    <div className="detail-val">6 months (2023–2024)</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Scope</div>
                    <div className="detail-val">
                      Client onboarding, loan lifecycle management, USSD
                      fallback, SMS/email notifications, admin dashboard
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Impact</div>
                    <div className="detail-val">
                      62% fewer branch visits; 4× faster loan approval cycle
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Users</div>
                    <div className="detail-val">3,200+ registered clients</div>
                  </div>
                </div>
              </div>
              <div className="project-meta">
                <span>📅 2024</span>
                <span>👤 3,200+ users</span>
                <span>⚡ 62% fewer visits</span>
              </div>
            </div>
          </div>

          <div
            className="project-card reveal"
            data-cat="mobile"
            data-id="logitrack"
          >
            <div className="project-thumb">🚚</div>
            <div className="project-body">
              <span className="project-tag tag-mobile">Mobile App</span>
              <h3>LogiTrack — Fleet & Delivery App</h3>
              <p>
                Real-time fleet tracking and delivery management app for a
                logistics company, with driver mobile app, dispatcher dashboard,
                and automated customer ETA notifications.
              </p>
              <div className="project-stack">
                <span className="stack-pill">React Native</span>
                <span className="stack-pill">FastAPI</span>
                <span className="stack-pill">Google Maps</span>
                <span className="stack-pill">Firebase</span>
              </div>
              <button
                className="expand-btn"
                onClick={() => toggleDetail("logitrack")}
              >
                {openIds["logitrack"]
                  ? "Hide details ↑"
                  : "View full details ↓"}
              </button>
              <div
                className={
                  "project-detail " + (openIds["logitrack"] ? "open" : "")
                }
              >
                <div className="project-detail-inner">
                  <div className="detail-row">
                    <div className="detail-label">Client</div>
                    <div className="detail-val">
                      Logistics & courier company, national coverage
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Duration</div>
                    <div className="detail-val">5 months (2022)</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Scope</div>
                    <div className="detail-val">
                      Driver app (iOS/Android), web dispatcher portal, live GPS
                      tracking, proof-of-delivery photos, invoicing
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Impact</div>
                    <div className="detail-val">
                      30% improvement in on-time delivery; customer complaints
                      down 55%
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Fleet size</div>
                    <div className="detail-val">80 vehicles tracked</div>
                  </div>
                </div>
              </div>
              <div className="project-meta">
                <span>📅 2022</span>
                <span>🚗 80 vehicles</span>
                <span>⚡ 30% on-time gain</span>
              </div>
            </div>
          </div>

          <div
            className="project-card reveal"
            data-cat="system"
            data-id="medicore"
          >
            <div className="project-thumb">🏥</div>
            <div className="project-body">
              <span className="project-tag tag-system">System</span>
              <h3>MediCore — Hospital Management System</h3>
              <p>
                A comprehensive hospital management system covering patient
                registration, appointment scheduling, EMR, pharmacy inventory,
                and billing — deployed across a private hospital network.
              </p>
              <div className="project-stack">
                <span className="stack-pill">Laravel</span>
                <span className="stack-pill">Vue.js</span>
                <span className="stack-pill">MySQL</span>
                <span className="stack-pill">PDF reports</span>
              </div>
              <button
                className="expand-btn"
                onClick={() => toggleDetail("medicore")}
              >
                {openIds["medicore"] ? "Hide details ↑" : "View full details ↓"}
              </button>
              <div
                className={
                  "project-detail " + (openIds["medicore"] ? "open" : "")
                }
              >
                <div className="project-detail-inner">
                  <div className="detail-row">
                    <div className="detail-label">Client</div>
                    <div className="detail-val">
                      Private hospital group, Harare & Bulawayo
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Duration</div>
                    <div className="detail-val">12 months (2021–2022)</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Scope</div>
                    <div className="detail-val">
                      Patient EMR, appointment booking, ward management,
                      pharmacy POS, insurance claims module, staff scheduling
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Impact</div>
                    <div className="detail-val">
                      Paperless records for 12,000+ patients; billing errors
                      reduced by 78%
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Locations</div>
                    <div className="detail-val">3 hospital sites</div>
                  </div>
                </div>
              </div>
              <div className="project-meta">
                <span>📅 2022</span>
                <span>🏥 3 sites</span>
                <span>⚡ 12,000+ patients</span>
              </div>
            </div>
          </div>

          <div className="project-card reveal" data-cat="web" data-id="edupath">
            <div className="project-thumb">🎓</div>
            <div className="project-body">
              <span className="project-tag tag-web">Web App</span>
              <h3>EduPath — LMS & Student Portal</h3>
              <p>
                A feature-rich learning management system for a private college,
                with video lectures, online assessments, student progress
                tracking, and fee management integrated into one platform.
              </p>
              <div className="project-stack">
                <span className="stack-pill">React</span>
                <span className="stack-pill">Django REST</span>
                <span className="stack-pill">PostgreSQL</span>
                <span className="stack-pill">AWS S3</span>
              </div>
              <button
                className="expand-btn"
                onClick={() => toggleDetail("edupath")}
              >
                {openIds["edupath"] ? "Hide details ↑" : "View full details ↓"}
              </button>
              <div
                className={
                  "project-detail " + (openIds["edupath"] ? "open" : "")
                }
              >
                <div className="project-detail-inner">
                  <div className="detail-row">
                    <div className="detail-label">Client</div>
                    <div className="detail-val">
                      Private tertiary institution, Harare
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Duration</div>
                    <div className="detail-val">7 months (2023)</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Scope</div>
                    <div className="detail-val">
                      Course authoring, video hosting, quiz engine, grade book,
                      fee payment, parent portal
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Impact</div>
                    <div className="detail-val">
                      800+ active students; admin workload reduced by 50%
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Courses</div>
                    <div className="detail-val">35 active courses</div>
                  </div>
                </div>
              </div>
              <div className="project-meta">
                <span>📅 2023</span>
                <span>👨‍🎓 800+ students</span>
                <span>⚡ 50% less admin work</span>
              </div>
            </div>
          </div>

          <div
            className="project-card reveal"
            data-cat="mobile"
            data-id="shopzim"
          >
            <div className="project-thumb">🛍️</div>
            <div className="project-body">
              <span className="project-tag tag-mobile">Mobile App</span>
              <h3>ShopZim — Local E-Commerce App</h3>
              <p>
                A marketplace mobile app connecting Zimbabwean buyers and SME
                sellers, with EcoCash and ZimSwitch payment integration,
                delivery partner API, and vendor analytics dashboard.
              </p>
              <div className="project-stack">
                <span className="stack-pill">Flutter</span>
                <span className="stack-pill">Node.js</span>
                <span className="stack-pill">MongoDB</span>
                <span className="stack-pill">EcoCash API</span>
              </div>
              <button
                className="expand-btn"
                onClick={() => toggleDetail("shopzim")}
              >
                {openIds["shopzim"] ? "Hide details ↑" : "View full details ↓"}
              </button>
              <div
                className={
                  "project-detail " + (openIds["shopzim"] ? "open" : "")
                }
              >
                <div className="project-detail-inner">
                  <div className="detail-row">
                    <div className="detail-label">Client</div>
                    <div className="detail-val">E-commerce startup, Harare</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Duration</div>
                    <div className="detail-val">9 months (2022–2023)</div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Scope</div>
                    <div className="detail-val">
                      iOS + Android app, vendor CMS, EcoCash/ZimSwitch payments,
                      delivery tracking, push notifications, admin panel
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">Impact</div>
                    <div className="detail-val">
                      5,000+ downloads in first 3 months; 120 active vendors
                    </div>
                  </div>
                  <div className="detail-row">
                    <div className="detail-label">GMV</div>
                    <div className="detail-val">
                      USD 180k+ processed in year one
                    </div>
                  </div>
                </div>
              </div>
              <div className="project-meta">
                <span>📅 2023</span>
                <span>📦 120 vendors</span>
                <span>⚡ 5k+ downloads</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

export default Projects;
